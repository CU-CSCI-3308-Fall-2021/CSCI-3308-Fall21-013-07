<?php
    ...
    if (isset($_POST['submit'])) {
        ...
        $fileActualExt = strtolower(end($fileExt)); // end() gets last index in an array

        $allowed = array('jpeg', "jpg", "png");

        if (in_array($fileActualExt, $allowed)) {
            if ($fileError === 0) {
                if ($fileSize < 200000) { // The file size is in kilobytes, NOT bytes
                    ...
                    if (empty($imgTitle) || empty($imgDesc)) {
                        ...
                        if (!mysqli_stmt_prepare($stmt, $sql)) {
                            ...
                            $sql = "INSERT INTO gallery (titleGallery, descGallery, imgFullNameGallery, orderGalleryColumn) VALUES(?, ?, ?, ?);";
                            if (!mysqli_stmt_prepare($stmt, $sql)) {
                                echo "SQL statement failed";
                            } else {
